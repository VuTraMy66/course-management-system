...
public interface SessionRepository extends JpaRepository<Sessions, Integer> {
    ...
    @Query("SELECT s FROM Sessions s WHERE s.course.courseId = :courseId ORDER BY s.duration ASC")
    List<Sessions> findSessionsByCourseOrderedByDuration(@Param("courseId") Integer courseId);
